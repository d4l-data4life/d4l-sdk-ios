...
class DocumentServiceTests: XCTestCase {
    ...
    func testCreateSingleDocument() {
        ...
        let document = BlobDocument(data: data)
...
fileprivate extension DocumentService {
    ...
    func fetchDocumentDelayed(withId identifier: String, key: Key, parentProgress: Progress) -> SDKFuture<BlobDocument> {

        return SDKFuture<BlobDocument> { promise in
            do {
